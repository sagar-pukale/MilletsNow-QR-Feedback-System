...
export const productController = {
  ...
  async create(request: Request, response: Response, next: NextFunction) {
    try { response.status(201).json(await productService.create({ ...request.body, imageUrl: request.file?.path ?? request.body.imageUrl })) } catch (error) { next(error) }
  },
  async update(request: Request, response: Response, next: NextFunction) {
    try { response.json(await productService.update(String(request.params.id), { ...request.body, ...(request.file ? { imageUrl: request.file.path } : {}) })) } catch (error) { next(error) }
  },
