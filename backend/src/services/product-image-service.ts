...
import { getSupabaseStoragePublicUrl, supabase, supabaseStorageBucket } from '../config/supabase.js'
...
export async function uploadProductImage(productId: string, file: Express.Multer.File) {
  ...
  const { error } = await supabase.storage.from(supabaseStorageBucket).upload(path, file.buffer, {
    cacheControl: '3600',
...
export async function removeProductImage(path?: string | null) {
  if (!path) return
  const { error } = await supabase.storage.from(supabaseStorageBucket).remove([path])
  if (error) {
