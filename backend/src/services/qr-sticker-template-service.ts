...
type StickerTemplateRecord = Prisma.QRStickerTemplateGetPayload<{
  include: {
    product: {
      select: {
        id: true
...
function toStickerText(template: {
  ...
  if (template.textMode === 'sku') return template.productSku
  if (template.textMode === 'product_name') return template.productName
  return `${template.productName} · ${template.productSku}`
}

async function resolveProductQrCode(productId: string, qrCodeId?: string | null) {
  if (qrCodeId) {
    ...
    if (!qr) throw new Error('Selected QR code was not found for this product.')
    return qr
  }
...
async function qrImageFor(template: {
  ...
  return QRCode.toDataURL(template.qrDestinationUrl, {
    margin: 1,
  ...
  for (let row = 0; row < pageConfig.rows; row += 1) {
    for (let column = 0; column < pageConfig.columns; column += 1) {
      ...
      page.drawRectangle({
        ...
        width: labelWidth,
        height: labelHeight,
        borderColor: rgb(0.88, 0.84, 0.79),
        borderWidth: 0.5,
      })
...
export const qrStickerTemplateService = {
  ...
  async create(input: unknown, publicBaseUrl: string) {
    const data = qrStickerTemplateSchema.parse(input)
    ...
    const row = await prisma.qRStickerTemplate.create({
      data: {
        name: data.name,
  ...
  async update(id: string, input: unknown, publicBaseUrl: string) {
    const templateId = stickerTemplateIdSchema.parse(id)
    const data = qrStickerTemplateUpdateSchema.parse(input)
    const existing = await prisma.qRStickerTemplate.findUnique({ where: { id: templateId } })
    ...
    const row = await prisma.qRStickerTemplate.update({
      where: { id: templateId },
      data: {
  ...
  async duplicate(id: string, publicBaseUrl: string) {
    ...
    const row = await prisma.qRStickerTemplate.create({
      data: {
        ...
        stickerConfig: (existing.stickerConfig ?? {}) as Prisma.InputJsonValue,
      },
      include: {
        product: {
          select: {
  ...
  async remove(id: string) {
    const templateId = stickerTemplateIdSchema.parse(id)
    try {
      await prisma.qRStickerTemplate.delete({ where: { id: templateId } })
      return true
    } catch {
      return false
    }
