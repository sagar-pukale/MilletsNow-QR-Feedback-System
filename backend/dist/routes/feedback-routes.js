...
feedbackRoutes.post('/', async (request, response, next) => {
    try {
        ...
        if (qr)
            await prisma.$executeRaw(Prisma.sql `UPDATE feedback SET product_id = ${qr.productId}::uuid, batch_id = ${qr.batchId}::uuid, qr_code_id = ${qr.id}::uuid WHERE id = ${created.id}::uuid`);
...
feedbackRoutes.get('/', requireAuth, async (request, response, next) => {
    try {
        ...
        const [rows, totals] = await Promise.all([
            prisma.$queryRaw(Prisma.sql `SELECT f.id, f.rating, f.message, f.status, f.submitted_at, p.name AS product_name, CASE WHEN q.id IS NOT NULL THEN 'question' WHEN c.id IS NOT NULL THEN 'compliment' WHEN co.id IS NOT NULL THEN 'complaint' ELSE 'feedback' END AS type FROM feedback f LEFT JOIN products p ON p.id = f.product_id LEFT JOIN questions q ON q.feedback_id = f.id LEFT JOIN compliments c ON c.feedback_id = f.id LEFT JOIN complaints co ON co.feedback_id = f.id ORDER BY f.submitted_at DESC LIMIT ${limit} OFFSET ${offset}`),
            prisma.$queryRaw(Prisma.sql `SELECT COUNT(*)::bigint AS count FROM feedback`),
        ]);
