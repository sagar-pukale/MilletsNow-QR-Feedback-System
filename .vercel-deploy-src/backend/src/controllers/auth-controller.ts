...
const cookieOptions = { httpOnly: true, sameSite: 'lax' as const, secure: env.NODE_ENV === 'production', maxAge: 86400000, path: '/' }
export const authController = {
  ...
  logout(_request: Request, response: Response) { response.clearCookie(authCookie, { httpOnly: true, sameSite: 'lax', secure: env.NODE_ENV === 'production', path: '/' }); response.status(204).send() },
