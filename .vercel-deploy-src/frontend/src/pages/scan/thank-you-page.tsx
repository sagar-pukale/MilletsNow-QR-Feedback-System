...
function CustomerThankYouPage() {
  ...
  const isCommonQrFlow = !qrToken
  const productName = (location.state as { productName?: string } | null)?.productName ?? (isCommonQrFlow ? 'Common QR' : 'MilletsNow')
  ...
  return (
    <CustomerPageShell>
      ...
      <section className="rounded-[2rem] border border-border/70 bg-white px-6 py-12 text-center shadow-card">
        ...
        <a
          href={isCommonQrFlow ? '/scan' : `/scan/${qrToken}`}
        ...
        >
          {isCommonQrFlow ? 'Back to feedback page' : 'Back to product page'}
        </a>
      </section>
