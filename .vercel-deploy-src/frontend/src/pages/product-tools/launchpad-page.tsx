...
function LaunchpadPage() {
  ...
  const configuredSocialItems = useMemo<SocialItem[]>(() => {
    const items: SocialItem[] = []
    if (socialLinks.instagram) items.push({ label: 'Instagram', href: socialLinks.instagram, icon: <span className="text-sm font-bold">IG</span> })
    if (socialLinks.facebook) items.push({ label: 'Facebook', href: socialLinks.facebook, icon: <span className="text-sm font-bold">f</span> })
    if (socialLinks.youtube) items.push({ label: 'YouTube', href: socialLinks.youtube, icon: <PlayIcon className="size-4" /> })
    if (socialLinks.whatsapp) items.push({ label: 'WhatsApp', href: socialLinks.whatsapp, icon: <MessageCircleIcon className="size-4" /> })
    return items
  }, [])
            ...
            {readyEntries.map(({ product, qrCode }) => (
              <Card key={product.id}>
                ...
                <CardContent className="space-y-4">
                  ...
                  {configuredSocialItems.length ? (
                    <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                      {configuredSocialItems.map((item) => (
                        <SocialLink key={item.label} label={item.label} href={item.href} icon={item.icon} />
                      ))}
                    </div>
                  ) : null}
                </CardContent>
