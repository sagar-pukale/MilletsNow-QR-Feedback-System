...
exports.productController = {
    ...
    async create(request, response, next) {
        try {
            response.status(201).json(await product_service_js_1.productService.create({ ...request.body, imageUrl: request.file?.path ?? request.body.imageUrl }));
        }
        catch (error) {
            next(error);
        }
    ...
    async update(request, response, next) {
        try {
            response.json(await product_service_js_1.productService.update(String(request.params.id), { ...request.body, ...(request.file ? { imageUrl: request.file.path } : {}) }));
        }
        catch (error) {
            next(error);
        }
