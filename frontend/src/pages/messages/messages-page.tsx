...
function MessagesPage() {
  ...
  useEffect(() => {
    ...
    // Subscribe to real-time feedback updates via Supabase Realtime channel
  ...
  const counts = useMemo(
    () => ({
      total: items.length,
      feedback: items.filter((item) => item.type === 'feedback').length,
      questions: items.filter((item) => item.type === 'question').length,
      compliments: items.filter((item) => item.type === 'compliment').length,
  ...
  const metrics = [
    ['Total messages', counts.total, MessageSquareIcon],
    ['Feedback', counts.feedback, MessageSquareIcon],
    ['Questions', counts.questions, HelpCircleIcon],
    ['Compliments', counts.compliments, HeartIcon],
    ['Complaints', counts.complaints, MessageSquareWarningIcon],
  ] as const
    ...
    <PageLayout className="bg-[#F7F8FA]">
      <PageContainer>
        <div className="space-y-6 pb-10">
          ...
          {loading ? (
            <Card>
              <CardContent className="p-8 text-center text-sm text-muted-foreground">Loading messages...</CardContent>
            </Card>
          ) : error ? (
            <Card>
            ...
            <EmptyState
              ...
              description="Customer messages will appear here once QR codes are scanned and customers get in touch."
              ...
              <CardContent className="divide-y p-0">
                {items.map((item) => (
                  <div key={item.id} className="flex flex-col gap-2 p-5 sm:flex-row sm:items-start sm:justify-between">
                    <div>
                      <p className="text-sm font-semibold">{item.message || 'Customer message'}</p>
                      <p className="mt-1 text-xs text-muted-foreground">
                        {item.productName ? `${item.productName} · ` : ''}
                        {item.type} · {new Date(item.submittedAt).toLocaleString('en-IN')}
                      </p>
                    </div>
