...
$loginBody = @{ email = "admin@milletsnow.example"; password = "change-this-development-password" } | ConvertTo-Json
...
Write-Host "Login response code: $($loginRes.StatusCode)" -ForegroundColor Green
$user = ($loginRes.Content | ConvertFrom-Json).user
...
$scanRes = Invoke-RestMethod -Uri "$baseUrl/scan/MN-LADOO-00001" -Method Get
...
try {
    Invoke-RestMethod -Uri "$baseUrl/scan/INVALID-TOKEN-999" -Method Get
} catch {
...
$fbBody = @{ qrToken = "MN-LADOO-00001"; rating = 5; message = "Delicious Millets Ladoo!"; type = "compliment" } | ConvertTo-Json
